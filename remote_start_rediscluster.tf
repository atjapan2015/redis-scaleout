...
resource "null_resource" "redis_master_add_masternode_rediscluster" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "for i in {1..${var.redis_rediscluster_shard_count}}; do for newmaster in $(cat /home/opc/master_list_$i.sh); do sudo -u root /usr/local/bin/redis-cli --cluster add-node $newmaster ${var.redis_server}:${var.redis_port} -a ${var.redis_password}; for newslave in $(cat /home/opc/replica_list_$i.sh); do sudo -u root /usr/local/bin/redis-cli --cluster add-node $newslave ${var.redis_server}:${var.redis_port} --cluster-slave --cluster-master-id $(echo 'cluster nodes' | redis-cli -c -h ${var.redis_server} -p ${var.redis_port} -a ${var.redis_password} | grep $newmaster | awk '{print $1}') -a ${var.redis_password}; done; done; done",
...
#      "for newslave in $(cat /home/opc/replica_list.sh); do sudo -u root /usr/local/bin/redis-cli --cluster add-node $newslave ${var.redis_server}:${var.redis_port} --cluster-slave -a ${var.redis_password}; done",

...
output "redis_master_public_ip_address" {
  value = {for i in range((var.redis_deployment_type == "Master Slave") ? var.redis_masterslave_master_count : ((var.redis_deployment_type == "Redis Cluster")?var.redis_rediscluster_shared_count : var.redis_standalone_master_count)) : i => data.oci_core_vnic.redis_master_vnic[i].public_ip_address}
}

output "redis_master_private_ip_address" {
  value = {for i in range((var.redis_deployment_type == "Master Slave") ? var.redis_masterslave_master_count : ((var.redis_deployment_type == "Redis Cluster")?var.redis_rediscluster_shared_count : var.redis_standalone_master_count)) : i => data.oci_core_vnic.redis_master_vnic[i].private_ip_address}
}

output "redis_replica_public_ip_address" {
  value = (var.redis_deployment_type == "Standalone")? {for i in range(var.redis_standalone_master_count) : i => data.oci_core_vnic.redis_master_vnic[i].public_ip_address} : ({for i in range((var.redis_deployment_type == "Master Slave") ? var.redis_masterslave_replica_count : var.redis_rediscluster_slave_count * var.redis_rediscluster_shared_count) : i => data.oci_core_vnic.redis_replica_vnic[i].public_ip_address})
}

output "redis_replica_private_ip_address" {
  value = (var.redis_deployment_type == "Standalone")? {for i in range(var.redis_standalone_master_count) : i => data.oci_core_vnic.redis_master_vnic[i].private_ip_address} : ({for i in range((var.redis_deployment_type == "Master Slave") ? var.redis_masterslave_replica_count : var.redis_rediscluster_slave_count * var.redis_rediscluster_shared_count) : i => data.oci_core_vnic.redis_replica_vnic[i].private_ip_address})
}
